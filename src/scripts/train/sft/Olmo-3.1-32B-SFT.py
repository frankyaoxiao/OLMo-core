"""
This script can be used to run SFT training for the 32B model.
...
DEFAULT_SEQUENCE_LENGTH = 8192  # 32B model uses 8K default for memory efficiency
DEFAULT_NUM_NODES = 1
GPUS_PER_NODE = 8
MAX_RANK_MICROBATCH_SIZE_TOKENS = 8192  # 1 sequence per microbatch (must be >= seq_len to avoid context parallelism)
    ...
    def build(
        ...
        *,
        ...
        ac_config = TransformerActivationCheckpointingConfig(
            mode=TransformerActivationCheckpointingMode.full,  # checkpoint all blocks for 32B
        )
        ...
        config = SFTConfig(
            ...
            train_module=TransformerTrainModuleConfig(
                ...
                optim=SkipStepAdamWConfig(
                    lr=2e-05,  # lower LR for larger model
            ...
            .with_callback(
                "checkpointer",
                CheckpointerCallback(
                    save_interval=500, ephemeral_save_interval=250, save_async=True
                ),
            )
            .with_callback(
                "wandb",
...
Examples:
  ...
  torchrun --nproc_per_node=8 %(prog)s train my-run /path/to/ckpt local \\
      --dataset_path=/path/to/data --save_folder=/path/to/checkpoints --seq_len=8192
""",
    )
    ...
    parser.add_argument(
        ...
        default=32 * DEFAULT_SEQUENCE_LENGTH,  # smaller batch for 32B model
    )
    parser.add_argument(
        "--save_folder",
        required=True,
